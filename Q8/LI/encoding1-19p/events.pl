...
%%%%%%  SAT Variables:
satVariable( ed(E, D) ) :- day(D), event(E).
satVariable( em(E, M) ) :- moderator(M), event(E).
satVariable( md(M, D) ) :- moderator(M), day(D).

writeClauses:-
    maxDaysMod,
    checkMaxDaysPerModerator,
    maxEventsDay,
    eachEventOneDay,
    eachEventOneMod,
...
eachEventOneDay :-
    event(E),
    findall(ed(E,D), (eventDays(E, D1), member(D,D1)), Lits),
    exactly(1, Lits),
    fail.
eachEventOneDay.
...
eachEventOneMod :-
    event(E),
    findall(em(E,M), (eventModerators(E,M1), member(M,M1)), Lits),
    exactly(1, Lits),
    fail.
eachEventOneMod.
%%%% Auxiliares
...
pertModerator(E,M1) :- eventModerators(E,M), pert(M1, M).

dayMod(D,M):- day(D), moderator(M).
