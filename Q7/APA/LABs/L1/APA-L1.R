...
##################
# Regressió lineal amb grau M=1, per començar (tots els models d'avui són lineals)
...
# Noteu que mean.square.error no és més que el nostre SSE dividit per n i multiplicat per 2
...
# com a curiositat, 'null.deviance' és l'error d'un model "null" (que només te el terme independent), i correspón al model M=0; l'AIC el veurem més endavant (o no)

model$null.deviance/n

# Dit tot això, reprenem el fil i tornem a l'error quadràtic mitjà. Sovint és convenient treballar amb la seva arrel, per obtenir així la "llargada" del vector d'errors, però en general no ho farem

(root.mse <- sqrt(model$deviance/n))

# El que sí farem és normalitzar l'error, dividint l'SSE per la variança dels targets, obtenint el que s'anomena NMSE (normalized mean squared error)
...
#   2) models amb NMSE > 1 són per tant horribles; un model es comença a considerar acceptable a partir de NMSE < 0.2
#   3) observeu que, notant que l'error quadràtic no és més que l'estimació de la variança dels targets, el NMSE es pot veure com la fracció de la variança dels targets NO explicada per les prediccions del model. Per exemple, un NMSE = 0.13 correspon a un model capaç d'explicar el 87% de la variabilitat del target.
...
# o sigui el polinomi és f(x) = 17.5885350·x^3 -26.5898537·x^2 -8.6171010·x + 0.3264768
...
# calculem l'error normalitzat NMSE a la mostra de TR
...
# Ara veurem com calcular l'error normalitzat a la mostra de validació
...
# Quan un model és incorrecte, el seu error de predicció és alt. En cas de models sobreajustats, sol ser molt superior al de TR. En cas de models infraajustats (com és el cas ara), tots dos són elevats i similars. Aquest és un model encara infraajustat però raonable i per tant tos dos errors són relativament baixos (notem que el millor model possible tindria error sigma.quadrat = 0.09; aquest model és el propi sin() i no és un polinomi).
...
# si no cap a la finestra, movem-ne els marges: és important veure la taula d'una sola peça
...
#   2) Això implica que a mesura que creix M, *tots* els coeficients es fan molt grans. I ens suggereix que, tot i sobreestimar el grau M, podríem controlar l'ajust si poguéssim limitar aquest creixement: això porta a la tècnica de regularització L_2 (que en estadística es coneix com a "ridge regression")
